...
public class ProblemaDAO implements IProblemaDAO {
    ...
    public boolean insertar(Problema problema) {
        String sql = "INSERT INTO Problema (descripcion, fch_ini, fch_fin, estado, idCliente) VALUES (?, ?, ?, ?, ?)";
        try (Connection conn = ConexionDB.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {

            ps.setString(1, problema.getDescripcion());
            ps.setDate(2, problema.getFchIni());
            ps.setDate(3, problema.getFchFin());
            ps.setString(4, problema.getEstado());
    ...
    public boolean actualizar(Problema problema) {
        String sql = "UPDATE Problema SET descripcion = ?, fch_ini = ?, fch_fin = ?, estado = ?, idCliente = ? WHERE idProblema = ?";
        try (Connection conn = ConexionDB.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
